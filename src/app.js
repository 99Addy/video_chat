...
const NotesWidget = () => {
  ...
  useEffect(() => {
    const handleCopiedText = (copiedText) => {
      console.log('React: Copied text received', copiedText);
      setCopiedTexts((prevCopiedTexts) => {
        let updatedCopiedTexts = [copiedText, ...prevCopiedTexts];
        updatedCopiedTexts = updatedCopiedTexts.filter((item, index) =>
           updatedCopiedTexts.indexOf(item) === index).slice(0, 10);
        localStorage.setItem('copiedTexts', JSON.stringify(updatedCopiedTexts));
        return updatedCopiedTexts;
      });
    };
  
    window.electron?.receive('copied-text', handleCopiedText);
  
    return () => {
      window.electron?.removeListener('copied-text', handleCopiedText);
    };
  }, []);

  useEffect(() => {
    const storedCopiedTexts = JSON.parse(localStorage.getItem('copiedTexts')) || [];
    setCopiedTexts(storedCopiedTexts);
  }, []);
    ...
    <div className="notes-widget">
      ...
      (!isPopupWindowActive && isMainWindowActive &&
        <div id="notes-widget-window" className="notes-widget-container">
          ...
          <div className="copied-texts">
            <h2>Last 10 Copied Texts</h2>
            <ul>
